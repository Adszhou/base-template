...
<ul class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-8">
  ...
  ?>
      <li class="post-item card rounded-xl shadow-xl p-4">
        <div class="my-4">
          <div class="space-x-2 font-news font-semibold text-lg">
            <?php foreach ($post_category as $category) : ?>
              <a class="italic hover:text-[#71efa3]" href="<?php echo get_category_link($category->term_id); ?>">
                <?php echo $category->name; ?>
              </a>
            <?php endforeach; ?>
          </div>
          <a class="text-xl line-clamp-2 min-h-[3.75rem] hover:text-[#71efa3] font-serif" href="<?php echo $post_link; ?>">
            <?php echo $post_title; ?>
          </a>
          <p class="text-xs text-gray-400 hover:text-[#71efa3]">
